...
import { STREET_MELODY_CHICKEN_PRELUDE_EVENT_COPY } from "@/lib/game/events";
...
type PreludePhase = "line-0" | "line-1" | "line-2" | "line-3" | "line-4" | "line-5" | "effect";
...
function getLine(phase: PreludePhase) {
  ...
  return STREET_MELODY_CHICKEN_PRELUDE_EVENT_COPY.lines[index] ?? null;
}

type StreetMelodyChickenPreludeEventModalProps = {
  onFinish: () => void;
  savings: number;
  actionPower: number;
  fatigue: number;
...
export function StreetMelodyChickenPreludeEventModal({
  onFinish,
  savings,
  actionPower,
  fatigue,
}: StreetMelodyChickenPreludeEventModalProps) {
  ...
  const sourceText = phase === "effect" ? STREET_MELODY_CHICKEN_PRELUDE_EVENT_COPY.effect : (line?.text ?? "");
    ...
    PHASE_ORDER.forEach((item) => {
      if (item === "effect") {
        if (PHASE_ORDER.indexOf(phase) >= PHASE_ORDER.indexOf("effect")) {
          lines.push({ id: "effect", speaker: "獲得", text: STREET_MELODY_CHICKEN_PRELUDE_EVENT_COPY.effect });
        }
        return;
      }
      ...
      if (PHASE_ORDER.indexOf(item) <= PHASE_ORDER.indexOf(phase)) {
        lines.push({ id: item, speaker: itemLine.speaker, text: itemLine.text });
      }
    });
    return lines;
  }, [phase]);
    ...
    <Flex position="absolute" inset="0" zIndex={50} direction="column" bgColor="#EDE7DE">
      ...
      >
        <Text color="#F5EFE5" fontSize="12px" textShadow="0 2px 6px rgba(0,0,0,0.45)">
          {STREET_MELODY_CHICKEN_PRELUDE_EVENT_COPY.sceneTitle}
        </Text>
      </Flex>
