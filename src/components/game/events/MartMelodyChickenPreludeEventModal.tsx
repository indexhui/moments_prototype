...
import { MART_MELODY_CHICKEN_PRELUDE_EVENT_COPY } from "@/lib/game/events";
...
function getLine(phase: PreludePhase) {
  ...
  return MART_MELODY_CHICKEN_PRELUDE_EVENT_COPY.lines[index] ?? null;
}

type MartMelodyChickenPreludeEventModalProps = {
  onFinish: () => void;
  savings: number;
  actionPower: number;
  fatigue: number;
...
export function MartMelodyChickenPreludeEventModal({
  onFinish,
  savings,
  actionPower,
  fatigue,
}: MartMelodyChickenPreludeEventModalProps) {
  ...
  const sourceText = phase === "effect" ? MART_MELODY_CHICKEN_PRELUDE_EVENT_COPY.effect : (line?.text ?? "");
      ...
      if (item === "effect") {
        if (PHASE_ORDER.indexOf(phase) >= PHASE_ORDER.indexOf("effect")) {
          lines.push({
            id: "effect",
            speaker: "獲得",
            text: MART_MELODY_CHICKEN_PRELUDE_EVENT_COPY.effect,
          });
        }
        return;
      }
    ...
    <Flex position="absolute" inset="0" zIndex={50} direction="column" bgColor="#EDE7DE">
      ...
      >
        <Text color="#F5EFE5" fontSize="12px" textShadow="0 2px 6px rgba(0,0,0,0.45)">
          {MART_MELODY_CHICKEN_PRELUDE_EVENT_COPY.sceneTitle}
        </Text>
      </Flex>
