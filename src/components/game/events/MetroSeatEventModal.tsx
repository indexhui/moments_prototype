...
type MetroSeatEventModalProps = {
  onFinish: () => void;
  savings: number;
  actionPower: number;
  fatigue: number;
  onChooseOption: (option: "sit" | "stand") => void;
};

export function MetroSeatEventModal({
  onFinish,
  savings,
  actionPower,
  fatigue,
  onChooseOption,
}: MetroSeatEventModalProps) {
  const {
    animation: backgroundShakeAnimation,
    effectNonce,
    activeEffectId,
  } = useBackgroundShake();
  const [step, setStep] = useState<MetroEventStep>("line-1");
  ...
  const sourceText = useMemo(() => {
    if (step === "line-1") return METRO_SEAT_EVENT_COPY.line1;
    if (step === "line-2") return METRO_SEAT_EVENT_COPY.line2;
  ...
  const chooseOption = (option: "sit" | "stand") => {
    ...
    setResultText(
      option === "sit"
        ? METRO_SEAT_EVENT_COPY.sitResult
        : METRO_SEAT_EVENT_COPY.standResult,
    );
    setEffectText(option === "stand" ? "疲勞值 -10" : "");
    setStep("result");
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text position="absolute" top="18px" color="#8B7B69" fontSize="12px">
          {METRO_SEAT_EVENT_COPY.sceneTitle}
        </Text>
      </Flex>
      ...
      {step === "choice" ? (
        <EventDialogPanel>
          ...
          <Flex
            ...
            onClick={() => chooseOption("sit")}
            cursor="pointer"
          >
            <Text color="white">坐著</Text>
            <Text color="white">行動力 +1</Text>
          </Flex>
          <Flex
            bgColor="rgba(255,255,255,0.1)"
            borderRadius="6px"
            p="10px"
            justifyContent="space-between"
            onClick={() => chooseOption("stand")}
            cursor="pointer"
          >
            <Text color="white">站著</Text>
            <Text color="white">行動力 -1</Text>
          </Flex>
        </EventDialogPanel>
      ) : (
        <EventDialogPanel>
          <Text color="white" fontWeight="700" pr="140px">
            {step === "line-2" ? "小麥" : "旁白"}
          </Text>
