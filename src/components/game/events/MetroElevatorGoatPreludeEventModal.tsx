...
import { METRO_ELEVATOR_GOAT_PRELUDE_EVENT_COPY } from "@/lib/game/events";
...
const ELEVATOR_BACKGROUND_IMAGE = "/images/428出圖/背景/捷運.png";

export type ElevatorPreludeChoice = "endure" | "turn";

type Phase =
  | { kind: "intro"; index: number }
  | { kind: "choice" }
  | { kind: "result"; choice: ElevatorPreludeChoice; index: number }
  | { kind: "effect"; choice: ElevatorPreludeChoice }
  | { kind: "hint"; choice: ElevatorPreludeChoice };

type MetroElevatorGoatPreludeEventModalProps = {
  onFinish: (choice: ElevatorPreludeChoice) => void;
  savings: number;
  actionPower: number;
  fatigue: number;
};

function getResultLines(choice: ElevatorPreludeChoice) {
  return choice === "endure"
    ? METRO_ELEVATOR_GOAT_PRELUDE_EVENT_COPY.endureResultLines
    : METRO_ELEVATOR_GOAT_PRELUDE_EVENT_COPY.turnResultLines;
}

function getEffectText(choice: ElevatorPreludeChoice) {
  return choice === "endure"
    ? METRO_ELEVATOR_GOAT_PRELUDE_EVENT_COPY.endureEffect
    : METRO_ELEVATOR_GOAT_PRELUDE_EVENT_COPY.turnEffect;
}

export function MetroElevatorGoatPreludeEventModal({
  onFinish,
  savings,
  actionPower,
  fatigue,
}: MetroElevatorGoatPreludeEventModalProps) {
  ...
  const currentLine = useMemo(() => {
    if (phase.kind === "intro") return METRO_ELEVATOR_GOAT_PRELUDE_EVENT_COPY.introLines[phase.index] ?? null;
  ...
  const sourceText = useMemo(() => {
    ...
    if (phase.kind === "hint") return METRO_ELEVATOR_GOAT_PRELUDE_EVENT_COPY.hint;
    return "";
  }, [currentLine, phase]);
  ...
  const historyLines = useMemo(() => {
    ...
    METRO_ELEVATOR_GOAT_PRELUDE_EVENT_COPY.introLines.forEach((line, index) => {
    ...
    if (phase.kind === "result" || phase.kind === "effect" || phase.kind === "hint") {
      ...
      if (phase.kind === "hint") {
        items.push({ id: "hint", speaker: "線索", text: METRO_ELEVATOR_GOAT_PRELUDE_EVENT_COPY.hint });
      }
    }
    return items;
  }, [phase]);
  ...
  const handleContinue = () => {
    ...
    if (phase.kind === "intro") {
      if (phase.index < METRO_ELEVATOR_GOAT_PRELUDE_EVENT_COPY.introLines.length - 1) {
  ...
  const handleChoose = (choice: ElevatorPreludeChoice) => {
  ...
  return (
    <Flex position="absolute" inset="0" zIndex={50} direction="column" bgColor="#EDE7DE">
      ...
      <Flex
        flex="1"
        bgImage={`url('${ELEVATOR_BACKGROUND_IMAGE}')`}
        bgSize="cover"
      ...
      >
        <Text color="#F5EFE5" fontSize="12px" textShadow="0 2px 6px rgba(0,0,0,0.45)">
          {METRO_ELEVATOR_GOAT_PRELUDE_EVENT_COPY.sceneTitle}
        </Text>
      </Flex>
      ...
      {phase.kind === "choice" ? (
        <EventDialogPanel>
          <Text color="white" fontSize="16px" fontWeight="700">
            {METRO_ELEVATOR_GOAT_PRELUDE_EVENT_COPY.choicePrompt}
          </Text>
          <Flex
            ...
            alignItems="center"
            cursor="pointer"
            onClick={() => handleChoose("endure")}
          >
            <Text color="white">{METRO_ELEVATOR_GOAT_PRELUDE_EVENT_COPY.options.endure.label}</Text>
            <Text color="#FCE9C8" fontSize="12px">
              {METRO_ELEVATOR_GOAT_PRELUDE_EVENT_COPY.options.endure.description}
            </Text>
          </Flex>
          <Flex
            bgColor="rgba(255,255,255,0.1)"
            ...
            alignItems="center"
            cursor="pointer"
            onClick={() => handleChoose("turn")}
          >
            <Text color="white">{METRO_ELEVATOR_GOAT_PRELUDE_EVENT_COPY.options.turn.label}</Text>
            <Text color="#FCE9C8" fontSize="12px">
              {METRO_ELEVATOR_GOAT_PRELUDE_EVENT_COPY.options.turn.description}
            </Text>
          </Flex>
        </EventDialogPanel>
      ) : (
