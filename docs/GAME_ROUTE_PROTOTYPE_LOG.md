...
- 新增 `narrativeMode` 場景設定，用來區分一般劇情與關鍵劇情。
- 關鍵劇情模式拆成可組合效果：
  - `visualStyle: "focus"`：只啟用場景聚焦演出。
  - `typingMode: "pause"`：需要時才切換慢速打字。
  - `continueDelayMs`：需要時才延遲繼續，不再與 key mode 綁死。
- scene-22 ~ scene-31 套用 `narrativeMode: { mode: "key", visualStyle: "focus" }`，讓吵架與交換日記伏筆段落有明確演出狀態，但不強制每句都慢速或鎖繼續。
- key scene 連續進 key scene 時，`NarrativeFocusLayer` 不再重播進場動畫，避免連續劇情每句都像重新開場。
- scene-22 小麥表情改為「嚴肅（開口）」；scene-28、scene-29 小麥表情改為「生氣（開口）」。

### scene-32 伏筆揭示演出

- scene-32 新增 `animals_runout` 背景，用來提前暗示交換日記與小日獸來源的關係。
- scene-32 停留時間由 360ms 延長為 1300ms，讓玩家能看清楚伏筆揭示。
- scene-32 加入專用揭示演出：
  - 左右兩片黑色斜切幕先覆蓋畫面，再向左右移開但不完全打開。
  - 保留部分黑色斜切版面，形成分鏡式構圖。
  - 「丟在地上的日記本」漫畫格延後浮現，並在揭示後短暫左右晃動，提醒玩家這本日記是關鍵線索。
...
- 現在改成：
  - 玩家完成黃金獵犬揭露（`hasSeenSunbeastFirstReveal === true`）後
...
### 2026-05 補充：第一次拍照教學與取景框手感

- 第一次黃金獵犬拍照進入 `scene-85` 前，會先顯示白話教學：
  - 等白色框框移到黃金獵犬身上。
  - 覺得位置差不多了，就按下快門！
- 教學確認後才開放快門按鍵，並在按鍵左側顯示 `public/images/pointer_up.png`：
  - 圖片旋轉為朝右，指向快門。
  - 用小幅重複位移提示玩家按鍵位置。
  - 不加陰影，避免 UI 過重。
- 第一次成功拍出可收下照片後，會提供一次免費重拍：
  - 玩家可選「免費重拍」或「收下這張」。
  - 若使用免費重拍，第二張成功後會顯示兩張照片比較，讓玩家選要留下哪一張。
- 取景框目前為 1:1 正方形，`CAMERA_FRAME_WIDTH` / `CAMERA_FRAME_HEIGHT` 同為 `248`。
- 取景框由上往下掃描，主線黃金獵犬入口使用：
  - `frameSweepFromY={20}`
  - `frameSweepToY={604}`
  - 讓 248px 正方形框能滑到手機舞台底部附近再循環。
- 取景框透明度不再用 keyframe 百分比猜時機。
  - 最終方法：每一幀讀取 `backgroundRef` 與 `cameraFrameRef` 的 DOM rect。
  - 依 `naturalImageSize`、舞台尺寸、`fitMode` 算出背景圖實際 render 後的位置。
  - 再把 `targetRectNormalized` 轉成畫面上的目標區。
  - 用「取景框底邊」和目標區上緣/下緣的距離即時計算 opacity。
  - 預設 `targetFadeLeadPx=50`，表示框底邊距離目標區上緣約 50px 時開始淡化。
- 這個做法比 keyframe 百分比穩定，因為它不依賴圖片比例、contain/cover offset、舞台尺寸或掃描起訖點的猜測。
...
本段為目前已完成的「第一次收集小日獸（黃金獵犬）」流程與技術落地，供下次接續「日記解鎖流程深化」與 UI polish。
...
## Update：2026-05-20 小雞小日獸特殊地圖流程

本次把小雞小日獸從舊版「線索概念」推進到可跑通的長流程。詳細規格已整理到 `docs/SUNBEAST_CHICKEN_EVENT_FLOW.md`。

### A) 已完成的玩家流程

- 街道地點新增 `打探 / 走走` 節奏。
- `打探 -> 商店街` 會進漫畫格與對話，並獲得特殊地圖。
- 獲得特殊地圖後：
  - `hasUnlockedSpecialMap = true`
  - `hasUnlockedSunbeastChickenHint = true`
  - 安排行程可切到特殊地圖 tab。
- 特殊地圖使用 `3x4` 近似棋盤、起點圖片、中央 mystery 格與可旋轉轉角拼圖。
- 特殊地圖出發後，過場中心 icon 使用問號，並進入小雞劇情地點。
- 小雞事件包含：
  - 街道初遇與第一次拍照
  - 老闆來電 overlay 與接聽互動
  - 小雞受驚逃走
...
第二段在玩家拿到特殊地圖前顯示鎖住狀態。舊版「同時經過轉角的公車和便利商店」已不再使用。

### C) 收服與日記

- 小雞收服沿用黃金獵犬的圖鑑 reveal 節奏：
  - 小雞內頁
  - 拍立得照片
  - `下一步`
  - 日記解鎖卡
  - `解鎖一篇日記`
- 小雞解鎖的是第三篇日記 `bai-entry-3`。
- 第三篇可以在第二篇仍鎖住時解鎖。
- 第三篇目前已有日記內文與小麥讀後對話。

### D) 這次暴露的技術債

- `ArrangeRouteView.tsx` 同時承擔一般棋盤、便利商店棋盤、特殊地圖、事件分流與過場銜接，之後應抽 board config 與 flow controller。
- `DiaryOverlay.tsx` 同時處理日記列表、日記內頁、小日獸圖鑑、收服 reveal、線索頁與讀後對話，之後應拆 presentational views 與小日獸資料表。
- 小日獸的線索、圖像、日記 id、收服文案仍散落在 JSX 與 progress 判斷中，下一隻小日獸前建議先做資料化。
- `EventPhotoCaptureLayer` 已可支援小雞場景，但場景參數仍是每個事件手動設定，之後可抽成 photo scenario config。
