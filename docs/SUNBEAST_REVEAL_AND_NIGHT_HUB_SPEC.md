...
這份文件整理夜間 Hub、黃金獵犬揭露、小日獸線索與回 Hub 對話的最新規格。
...
- 這個模式不是一般圖鑑瀏覽，它會帶首次揭露、黃金獵犬內頁解鎖、follow-up 對話

### 首次可遊玩的回家 Hub 三功能教學

黃金獵犬與第一篇日記揭露完成後，第一次回到可操作的夜間 Hub，要先播放一次三步功能教學。

觸發條件：

- `unlockedDiaryEntryIds` 包含 `bai-entry-1`
- 已拍到 / 收集黃金獵犬，或仍有 `hasPendingFirstSunbeastNightHubGuide`
...
2. 指向右側 `小日獸`：
   - 小貝狗：`小日獸裡可以查看黃金獵犬，還有日記裡出現過的相關小日獸線索。`
3. 指向左下 `睡覺`：
   - 小麥：`睡覺來休息一下，準備明天的行程吧。`
...
3. 第一格從 `???` 變成黃金獵犬
4. 小麥對話：
   - 「黃金獵犬出現在日記上了！對，是小白常常提到的黃金獵犬。」
5. 第二句引導：
   - 「點點看黃金獵犬吧。」
6. 玩家點進黃金獵犬內頁
7. 黃金獵犬頁先顯示拍立得貼上 / 淡入演出
8. 之後進入解鎖演出：
   - 日記
   - 地點（街道）
   - 小日獸線索（青蛙 / 小雞）
...
山羊沿用「上一隻小日獸完成後先揭露下一篇拼圖，正式收集後再還原全文」的流程。

### 公雞完成後

1. 公雞完整日記讀完後解鎖 `bai-entry-4`。
2. 玩家點開第四篇時直接看到四片可交換、可拖曳的圖片拼圖；拼圖頁不顯示日期或日記標題。
3. 拼圖階段只保留前半段日記文字，不可先顯示完整內容。
4. 拼圖操作提示使用固定高度的保留槽；完成後只隱藏提示內容，不移除該空間，避免圖片與文字向上跳動。
5. 拼圖片與文字必須使用完整日記正文欄的相同座標；最後一片歸位後，完成圖片不可停留在較寬、較低的解謎版位。
6. 拼圖完成後直接顯示「繼續」，不在拼圖頁補回日期或日記標題。
7. 讀後對話完成後進入山羊篇。

### 山羊收集後

1. 確認山羊照片後不插入額外的 `NEW SUNBEAST／查看收服紀錄` 卡，直接打開 `DiaryOverlay`，先播放拍立得照片由左往右飛入日記的共通演出。
2. 接著顯示山羊圖鑑與拍立得照片。
3. 玩家按「還原日記」後直接進入第四篇內頁，不得先切到交換日記目錄要求玩家再點一次第四篇。
4. 第四篇內頁依序還原圖片、後半段文字與正式標題。
5. 後半段文字在揭露時使用青綠色，標題落定後沉回一般日記色。
6. 標題完成後才顯示「繼續」，再播放完整日記的讀後對話。
7. 讀後對話完整播完才離開第四篇；回到交換日記目錄時，列表要自動捲到最新浮現的篇章並顯示點擊指標。
8. 不得回到舊版「下一篇日記片段／???／只看得見一小段」頁面。

## 8.3 山羊完成後的海豹殘頁

山羊完整日記讀完後，下一隻小日獸的前導是海豹；此階段只揭露家中的第一段日記，不提前播放清掃事件或海豹收服。

1. 山羊讀後對話完成後解鎖 `bai-entry-6`。
2. 交換日記列表先顯示新殘頁卡與小麥提示，再由玩家點開。
3. 點開後先進入四片直向圖片拼圖；玩家可拖曳交換，也可依序點兩片交換位置。
4. 拼圖頁使用米色斜紋外框、`#657179` 藍灰線白色內頁、同色拼圖框線與分隔線、藍灰色 `？？？` 標題帶及圓角按鈕，貼近海豹首張日記示意圖。
5. 插圖沿用 Figma 的懶人骨頭沙發搬家圖；拼圖未完成前，正文以散落文字格呈現，不可直接閱讀完整內容。
6. 四片歸位後，在同一張紙頁還原完整插圖與「逛家具店、坐上沙發後買回家」的第一段正文，完成演出後才顯示「確定」。
7. 沙發成為雜物堆、搬回房間與清掃後的反思，保留給後續海豹篇事件揭露。
8. 玩家按「確定」後，以跨日轉場進入海豹篇的白天客廳開場，依序交代小麥被工作折磨一週、小白仍未甦醒、請假休息與準備大掃除；不再回到通用夜間 Hub。
...
玩家完成黃金獵犬揭露，回到夜間 Hub 後，再依照 Hub 引導點進 `小日獸`：
...
- 小麥：意識到不只黃金獵犬，連街道和其他小日獸的出現方式也開始有方向
