...
Figma 版面參考：`走走小日 2026`；遊戲節點 `12522:1982`、完成卡節點
`12620:173`、箱子與青蛙動畫節點 `12609:2324`。
...
6. 通關 modal 依剩餘 1／2／3 顆愛心顯示「成功／大成功／超成功」。
7. 按下「完成」後，背景切成裝滿傳單的箱子，依序播放青蛙影格
   `1 → 2 → 3 → 4 → 5 → 6 → 停留 1 秒 → 7 → 8 → 9`。
8. 第 9 格播放完畢後直接進入拍青蛙，不再插入事件對話或拍照教學 modal。
9. 拍照階段沿用傳單箱子背景，兩張青蛙素材會先完成載入與解碼，再以 `280 ms` 交替；
   青蛙沿較大的橢圓路徑巡遊，可移動到箱子的上、右、下、左四側。
10. 兩張縮小的傳單會以相反相位繞著青蛙漂浮；玩家可用滑鼠、手指或方向鍵稍微調整位置，
    放開後傳單會從新位置繼續漂浮，拍攝時的狀態也會一併寫入拍立得。
11. 此階段只能按下拍照按鈕觸發快門；點背景與空白鍵都不會拍照。
...
5. 第 8 張起是否同時出現兩條風道。
6. 成功、失敗、重試與完成按鈕是否都能完成。
7. 三種剩餘生命是否分別顯示成功、大成功、超成功。
8. 完成後是否依指定順序播放箱子／青蛙動畫，並直接進入拍照。
9. 拍照階段的青蛙與傳單是否保持在箱子附近，傳單拖曳後是否繼續漂浮。
10. 背景點擊與空白鍵是否不會觸發快門，只有拍照按鈕可以拍照。
11. `npm run build` 與 `npx tsc --noEmit` 是否通過。
