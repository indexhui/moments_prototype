...
**2026-09-25 改善版覆寫**：本文紙膠帶相關要求是東京電玩展版本的比較基準。改善版三篇上段改為「圖片拼圖完成 → 含地點的正文自動還原 → 文字落定後繼續 → 取得地點線索」，移除紙膠帶收集、浮動書籤、正文缺字輪廓及拖放門檻。三篇上下段、照片揭露與地點順序維持不變。2026-09-26 起，改善版拼圖期間不顯示文字，完成後沿用黃金獵犬的圖片上移與字片重組演出；第一層完成並歸位後，才淡入第二層縮圖預告，再放大移入。Unity／Steam 改善版以 [改善紀錄第 08～10 項](EXHIBITION_IMPROVEMENT_LOG.md) 為準。
...
| --- | --- | --- | --- | --- | --- |
| 第一篇 | `今天是和小麥正式成為室友的日子！`<br>`我們約了搬家公司，只是在門口等搬家公司來的時候，街道忽然傳來一陣騷動……` | `街道` | 街道，`0/3 → 1/3` | `原來是鄰居家小孩玩球時，不小心撞上了發傳單的人，傳單飛得到處都是！`<br>``反正搬家公司也還沒來，我就和小麥一起幫忙把四散的傳單撿回來，算是做了一樁好事呢(´ω` )``<br>插圖依序加入 `kid`、`bai` 圖層。 | 按繼續進第二篇上段拼圖。 |
| 第二篇 | `傳單撿好後沒多久，搬家公司也來了！天氣很熱、東西又很多，搬到超級累的（「°Д°）「，後來在休息時，我看到櫃子上有幾瓶便利商店的飲料……`<br>圖層拼圖依序為 `empty`、`stack`，完成後加入 `baidrink`。 | `便利商店` | 便利商店，`1/3 → 2/3` | `我想，那些飲料應該是小麥買回來的吧？`<br>`真的是太貼心了！因為口實在太渴，忍不住直接開了幾瓶，咕嚕咕嚕地一口氣乾`<br>插圖依序加入 `worker`、`bai_head` 圖層。 | 按繼續進第三篇上段拼圖。 |
| 第三篇 | `沒想到搬家到一半時，我才知道……那些飲料居然是搬家工人買來要自己喝的!!!! (;´༎ຶД༎ຶ\`) 我真的是!!!丟臉到爆炸!!!! 後來出去領要給搬家費用的錢，經過我最愛的甜點店時……`<br>圖層拼圖先解四片 `empty`，再解三片直向 `table`。 | `甜點店` | 甜點店，`2/3 → 3/3` | `小麥提議，可以買一些小點心回去賠罪，所以我們就買了幾個蛋糕回去請搬家工人吃，幸好他們人很好，沒有計較我喝了他們的飲料！下次真的不可以再這麼丟臉了（「´д\`）`<br>第三段浮出時加入 `frog_diary_03_final.png`。 | 青蛙正式收集，完成〈搬家〉並進展覽收尾。 |

三篇青蛙圖層拼圖皆不繪製圖片切片分隔線或外框；交換操作保留，點選片只以輕微亮度變化提示，拖曳時才暫時顯示落點。
