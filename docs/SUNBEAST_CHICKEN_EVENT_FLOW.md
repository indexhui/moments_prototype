...
2. 拍照前小麥與小貝狗對話。
3. 第一次拍照：
   - 使用 `/images/背景/公司附近街道_白天.jpg` 作為乾淨背景。
   - 公雞使用 `/images/animals/公雞.png` 疊在背景上。
   - 背景比畫面寬，支援左右取景與縮放。
   - 拍照按鈕是一整條快門區。
4. 老闆來電：
   - 顯示黑色遮罩與整個來電 overlay。
...
- `npm run build` 通過。
- 黃金獵犬首次揭露後，青蛙與小雞仍會以 `hint` 狀態出現在小日獸圖鑑。
